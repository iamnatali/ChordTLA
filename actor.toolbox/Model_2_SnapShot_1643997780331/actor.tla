      ...
      if (actorInboxes[self] /= <<>>) {
        ...
        ProcessMessage:
         if (kind = "FindPredecessor") {
           ...
           print self;
           print id;
           print fingerTables[self][self + 1];
           print between01(self, id, fingerTables[self][self + 1]);
           if (between01(self, id, fingerTables[self][self + 1])){
            ...
            print "hey";
            FindFistSuitableI:
            while (i > 0 /\ ~(i \in DOMAIN fingerTables[self])){
             i := i - 1;
            };
            print "i";
            print i;
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "e965c8b7" /\ chksum(tla) = "64dc1495")
...
ProcessMessage(self) == /\ pc[self] = "ProcessMessage"
                        /\ IF kind[self] = "FindPredecessor"
                              THEN /\ id' = [id EXCEPT ![self] = currentMessage[self][2]]
                                   ...
                                   /\ PrintT(self)
                                   /\ PrintT(id'[self])
                                   /\ PrintT(fingerTables[self][self + 1])
                                   /\ PrintT(between01(self, id'[self], fingerTables[self][self + 1]))
...
FindFistSuitableI(self) == /\ pc[self] = "FindFistSuitableI"
                           /\ IF i[self] > 0 /\ ~(i[self] \in DOMAIN fingerTables[self])
...
MainLoop(self) == /\ pc[self] = "MainLoop"
                  /\ IF i[self] > 0 /\ ~(between00(self, fingerTables[self][self + (2^(i[self]-1))], id[self]))
                        ...
                        ELSE /\ IF i[self] = 0
                             ...
                             /\ pc' = [pc EXCEPT ![self] = "WaitForMessages"]
                  /\ UNCHANGED << fingerTables, triggered, m, stack, 
                                  trigger_content, currentMessage, kind, 
                                  content, id, asker, i >>
...
FindSuitableI(self) == /\ pc[self] = "FindSuitableI"
                       ...
                       /\ UNCHANGED << actorInboxes, fingerTables, triggered, 
                                       ...
                                       currentMessage, kind, content, id, 
                                       asker >>

actor(self) == WaitForMessages(self) \/ ProcessMessage(self)
                  \/ FindFistSuitableI(self) \/ MainLoop(self)
                  \/ FindSuitableI(self)
...
\* Last modified Fri Feb 04 23:02:53 YEKT 2022 by pervu
