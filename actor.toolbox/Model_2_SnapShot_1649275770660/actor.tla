  ...
  gotPredecessor = "no15";
{
  Join:
    if (self = 0){
        ...
        await predecessorAnswer # "no" /\ predecessorSuccessorAnswer # "no";
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "dabf0978" /\ chksum(tla) = "f4ab7199")
...
Join(self) == /\ pc[self] = "Join"
              /\ IF self = 0
                    ...
                    ELSE /\ actorInboxes' = [actorInboxes EXCEPT ![0] = Append(actorInboxes[0], <<"FindPredecessor", fingerStart(self, 1, bm), self>>)]
                         /\ predecessorAnswer[self] # "no" /\ predecessorSuccessorAnswer[self] # "no"
...
\* Last modified Thu Apr 07 01:09:22 YEKT 2022 by pervu
